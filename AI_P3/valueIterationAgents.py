...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def __init__(self, mdp, discount = 0.9, iterations = 100):
        ...
        state = mdp.getStates()
        action = mdp.getPossibleActions(state[1])
        nextState = mdp.getTransitionStatesAndProbs(state[1], action[0])
        # print(mdp.getPossibleActions(state[0]))
        # for i in state:
        #     if(len(mdp.getPossibleActions(i))==0):
        #         print(i)
        # print(mdp.getTransitionStatesAndProbs(state[1], action[0]))
        # print(mdp.getReward(state[1], action[0], nextState[0]))
...
class AsynchronousValueIterationAgent(ValueIterationAgent):
    ...
    def runValueIteration(self):
        ...
        for i in range(self.iterations):
            ...
            max_val = float('-inf')
            for action in actions:
                Q_val = self.computeQValueFromValues(curr_state,action)
                max_val=max(max_val,Q_val)
            self.values[curr_state]=max_val
